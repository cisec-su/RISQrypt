...
{
    return x2x_core(dst_1, dst_0, 0x0, src, len, X2X_CTRL_CONV_MODE_A2B, X2X_CTRL_SRC_SHARE_V, 0, 0, X2X_CTRL_CMD_X2X);
}


int x2x_a_share(uint32_t *dst_1, uint32_t *dst_0, uint32_t *src, unsigned int len)
{
    return x2x_core(dst_1, dst_0, 0x0, src, len, X2X_CTRL_CONV_MODE_B2A, X2X_CTRL_SRC_SHARE_V, 0, 0, X2X_CTRL_CMD_X2X);
}
