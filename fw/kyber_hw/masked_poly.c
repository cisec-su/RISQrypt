 ...
 * Distinctively, we use mod c+alpha+1 during decompress.
 */
static void masked_poly_compress(masked_ptr r, const masked_poly *a, uint32_t d) {
    ...
    for (i = 0; i < MASKING_N - 1; i++) {
        ntt_lite_decode(NTT_LITE_OUTPUT_DIS, (uint32_t*) a->share[i].coeffs, 16);
        ntt_lite_compress(mpu32.share[i].coeffs, NTT_LITE_INPUT_DIS, d_);
    }
    ntt_lite_decode(NTT_LITE_OUTPUT_DIS, (uint32_t*) a->share[MASKING_N - 1].coeffs, 16);
    ntt_lite_compress(NTT_LITE_OUTPUT_DIS, NTT_LITE_OUTPUT_DIS, d_);
    ...
    for (i = 0; i < MASKING_N; i++) {
        ...
        ntt_lite_encode(NTT_LITE_OUTPUT_DIS, mpu32.share[i].coeffs, 16);
        ntt_lite_load_q(1, &mu, 7, KYBER_DV, inv2, NTT_LITE_MODE_POLY);
        ntt_lite_decompress_floor(NTT_LITE_OUTPUT_DIS, NTT_LITE_INPUT_DIS, alpha);
        ntt_lite_encode((uint32_t*) r[i], NTT_LITE_INPUT_DIS, KYBER_DV);
    }

}


void masked_poly_compress_dv(masked_ptr r, const masked_poly *a) {
    masked_poly_compress(r, a, KYBER_DV);
}


void masked_poly_compress_du(masked_ptr r, const masked_poly *a) {
    masked_poly_compress(r, a, KYBER_DU);
}
