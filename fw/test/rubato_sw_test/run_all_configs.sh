...
PARAMS_SOURCE="$SCRIPT_DIR/../../src/rubato_sw/params.h"
...
if [[ -z "$PROJ_NAME" ]]; then
  PROJ_NAME="rubato_sw_test"  # fallback
fi
...
text = re.sub(r'^#define\s+PARAM_(80|128)_(S|M|L)\s*$', f'#define {param}', text, flags=re.M)
...
run_one() {
  ...
  cycle="$(printf '%s\n' "$run_output" \
    | sed -n 's/.*RUBATO_SOFT_ENCRYPT:[[:space:]]*\([0-9][0-9]*\)[[:space:]]*cycles.*/\1/p' \
    | head -n1)"
  if [[ -z "$cycle" ]]; then
    cycle="NA"
  fi
  ...
  masked_cycle="$(printf '%s\n' "$run_output" \
    | sed -n 's/.*MASKED[[:space:]_]*RUBATO_SOFT_ENCRYPT:[[:space:]]*\([0-9][0-9]*\)[[:space:]]*cycles.*/\1/p' \
    | head -n1)"
