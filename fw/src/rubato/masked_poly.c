...
/**
 * @brief Masked multiply-add with unmasked multiplicand: D = a*B + C
 * @description Computes D = a*B + C where a is an unmasked (public) polynomial and B, C are masked polynomials. Because a is public, pointwise multiplication is linear in each share and requires no refresh randomness: d.share[i] = a * b.share[i] + c.share[i] for every share i. Uses ntt_lite_pwm for pointwise multiplication and ntt_lite_add for the accumulation, chaining through the accelerator I/O registers
 * @param d pointer to output masked polynomial
 * @param a pointer to unmasked input polynomial (public multiplicand)
 * @param b pointer to input masked polynomial (multiplicand)
 * @param c pointer to input masked polynomial (addend)
 * _umm represents the case where the FIRST input is unmasked, SECOND and THIRD inputs are masked.
 * @return void
 */
