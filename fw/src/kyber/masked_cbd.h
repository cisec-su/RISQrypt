...
void masked_cbd_eta2_i(masked_polyvec *r, const uint32_t buf[MASKING_N][(KYBER_N*KYBER_ETA2*2) >> 5], unsigned int i);

#define masked_cbd_eta2 KYBER_NAMESPACE(_masked_cbd_eta2)
void masked_cbd_eta2(masked_poly *r, const uint32_t buf[MASKING_N][(KYBER_N*KYBER_ETA2*2) >> 5]);
