...
void masked_polyvecl_pointwise_acc(masked_poly_ptr *w, const polyvecl *u, const masked_polyvecl_t *v)
{
    unsigned int i, j;
    uint32_t *dst;
...
void masked_polyveck_decompose(polyveck *v1, masked_polyveck_t *v0, const masked_polyveck_t *v) {
    unsigned int i, j;
    masked_poly_ptr_const v_ptr;
    masked_poly_ptr v0_ptr;
    
    for (i = 0; i < K; i++) {
        masked_polyveck_t_to_poly_ptr_const(&v_ptr, v, i);
        masked_polyveck_t_to_poly_ptr(&v0_ptr, v0, i);
...
int masked_polyvecl_pointwise_add_invntt_chknorm(masked_polyvecl_t *r, const masked_polyvecl_t *v, const poly *c, masked_polyvecl_t *u, uint32_t B) {
    ...
    for (i = 0; i < L; i++) {
        ...
        masked_polyvecl_t_to_poly_ptr_const(&v_ptr, v, i);
...
int masked_polyveck_pointwise_invntt_sub_chknorm(masked_polyveck_t *r, const masked_polyveck_t *v, const poly *c, const masked_polyveck_t *u, masked_poly_ptr *temp, uint32_t B) {
    ...
    for(i = 0; i < K; i++) {
        masked_polyveck_t_to_poly_ptr(&r_ptr, r, i);
        masked_polyveck_t_to_poly_ptr_const(&u_ptr, u, i);
        masked_polyveck_t_to_poly_ptr_const(&v_ptr, v, i);
...
void masked_polyveck_unmask(polyveck *a, const masked_polyveck_t *r) {
    ...
    for (i = 0; i < K; i++) {
        masked_polyveck_t_to_poly_ptr_const(&r_ptr, r, i);
...
void masked_polyvecl_unmask(polyvecl *a, const masked_polyvecl_t *r) {
    ...
    for (i = 0; i < L; i++) {
        masked_polyvecl_t_to_poly_ptr_const(&r_ptr, r, i);
        masked_poly_ptr_unmask(&a->vec[i], &r_ptr);
    }
}