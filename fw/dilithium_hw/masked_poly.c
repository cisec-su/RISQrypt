...
void masked_poly_decompose(poly *v1, masked_poly *v0, const masked_poly *v) {
    ...
    for (i = 0; i < MASKING_N; i++) {
// #ifdef TTEST
//         // v1 corresponds to lsb log(gamma)-bits of temp and v1 is public.
//         // Therefore, in real implementation, there is no harm in leaking v1.
//         // However, to avoid unintended leakage in t-test setting, we clear ntt_lite state
//         // to avoid transitional leakage between shares of v1
//         // also note that the transitional leakage discussed here corresponds to only the
//         // lsb log(gamma)-bits of temp, not the other bits.
